...
class LoginManager:
	...
	def authenticate(self, user: str = None, pwd: str = None):
		...
		customer = frappe.get_all("Customer" ,{"user_id":user.name}, ["name"])
		if customer:
			subscription = frappe.get_all("Subscription", {"party_type":"Customer", "party":customer[0].name, "end_date":[">=",frappe.utils.nowdate()]}, ["end_date"])
			if not subscription:
				frappe.throw(f"Your Subscription End Date is Finised")
